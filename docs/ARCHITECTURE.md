...
缺少结束时间不等于正在持续：查询排序和范围相交以 `end ?? start` 处理已知时间点，原始文档、interval 和详情继续保留 null。甘特图把这类记录的图示终点延伸到该轴全部标准化记录的最大已知坐标，包含未分组记录，但不为未分组记录制造事件栏；这就是当前视图使用的“当前剧情时间”，并非独立推进的剧情时钟。零跨度轴上的标记位于右端，不伪造时长。条纹标记与 end=start 的确定零时长圆点区分。事件编辑表单以同一时间线为前提复用已有 start，空 end 显式提交 null，空 start 不转换成 0。
