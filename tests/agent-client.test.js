...
test('client bundle keeps additive controls and scopes exact assistant replacement to Tavern sessions', async () => {
  ...
  try {
    ...
    let nextDraft
    const composerElement = slotComponents[0]({ sessionId: 'session-a', ...slotOptions[0].inject(), input: { draft: '已有草稿', phase: 'plain' }, inputActions: { setDraft(value) { nextDraft = value } }, useSessions: selector => selector(sessionState) })
    assert.equal(composerElement.type.name, 'TavernCharacterSelect')
    const legacyComposerElement = slotComponents[0]({ sessionId: 'legacy-session', input: { draft: '', phase: 'plain' }, inputActions: { setDraft() {} }, useSessions: selector => selector({ byId: { 'legacy-session': { agentPreset: 'sillytavern' } } }) })
    ...
    assert.equal(slotComponents[0]({ sessionId: 'session-b', blocks: conversation.blocks, useSessions: selector => selector({ byId: { 'session-b': { projectionValues: { agentPreset: 'cat' } } } }) }), null)
    ...
    const htmlFrame = mixedGreeting.children[1].type(mixedGreeting.children[1].props)
    ...
    assert.match(htmlFrame.props.srcDoc, /window\.setChatMessages=setChatMessages/)
    assert.match(htmlFrame.props.srcDoc, /window\.triggerSlash=triggerSlash/)
    assert.match(htmlFrame.props.srcDoc, /__dshSillyTavernGreeting:true/)
    ...
    const embeddedGreetingFrame = embeddedFenceGreeting.children[1].type(embeddedFenceGreeting.children[1].props)
    ...
    assert.match(scriptsTabSource, /管理页不会自动执行/)
    ...
    assert.match(source, /composerBridges\.get\(sessionId\)/)
    ...
    assert.doesNotMatch(scriptsTabSource, /script\.enabled && script\.source\.trim\(\).*TrustedFrame/)
    assert.match(source, /patchTrusted\(index, script\.id, \{ source: event\.target\.value \}\)/)
    assert.match(source, /const srcDoc = React\.useMemo\(\(\) => trustedDocument/)
    ...
    assert.match(source, /onLoad: \(\) => setReady\(true\)/)
    ...
    assert.match(source, /\.dst-opening-html\{[^}]*flex:1 1 360px/)
    ...
    assert.match(source, /@container \(max-width:780px\).*max-width:52px/)
    assert.match(source, /@container \(max-width:560px\).*max-width:28px/)
