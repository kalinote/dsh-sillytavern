...
- 新增可复用的复杂时间线演示数据：6 条时间线、37 个事件、79 条记忆与 50 条先后关系，涵盖跨轴事件、并行汇合、不连续区间、未结束事件及单坐标轴，并提供浏览说明。
- 甘特图的事件聚焦改用放大镜图标，悬停显示按钮名称；第一个刻度始终左对齐，零跨度轴的唯一刻度和事件标记也放在左端。
- 甘特图新增每轴独立缩放、平移、总览和事件时间聚焦；极短区间保留可点击标记，局部放大仍按原始坐标线性显示。未记录结束时间的条形统一延伸到该轴最新已知剧情时间，边界包含未分组记录。
- 维护提示要求保留剧情给出的秒/毫秒等细节，并按世界观的实用显示精度补齐 label 低位，同时注明原文粗精度及补零。亿年等长时间也只补显示，坐标和数值单位保持不变，不增加 schema、兼容或迁移代码。

- 移除剧情时间长整数的自动公历识别、日期格式化和 epoch 绘图换算；时间轴刻度、间隔与条形长度统一使用原始数值，label 保留世界观时间名称，小数坐标不再按四位小数舍入。维护提示和查询工具说明明确同一时间线的原点、单位与数值语义；schema 5 不变，不添加兼容或迁移逻辑。

- 修复时间线将同一事件的每条记忆重复画成独立事件行的问题：每个事件在同一时间线上只占一行，相同时间坐标共用标记，不同时间段保留在该行；显示记忆数量，完整记忆、时间标签和搜索仍保留，未知时间区也按事件聚合。

- 将回复下方的重生成、编辑行动后重试、从这里继续改为与原生操作栏一致的图标按钮，保留悬停说明、键盘焦点提示与不可用原因。

- 修复带 body 外边距的角色卡 iframe 高度反复增减、滚动条闪烁的问题；尺寸计算包含底部边距及文档滚动偏移，选项展开与折叠仍可双向调整高度，空内容也不会因默认边距反复撑开。
- 修复新版 DSH 将系统提示放入 `messages` 后，世界书 `@Depth` 状态栏和选项激活却未注入的问题；兼容旧 `system` 字段，保留冻结的原始消息，按请求绑定独立投影。世界书诊断增加实际深度插入状态与数量。
- 修复酒馆标签污染普通会话：时间线、世界书、用户设定/变量、脚本和提示词模板仅在当前会话使用 `sillytavern` 预设时注册，切换会话或预设时同步挂载/注销，插件卸载时清理订阅及标签。
- 修复酒馆脚本的就绪、切页断开和重连生命周期；空消息渲染区域按内容高度收起，异常区域提供加载状态与恢复入口。
- 分支按来源剧情与分支点继承角色和运行状态；旧分支可恢复来源配置，缺少历史快照时明确提示无法精确还原。失败回合保留玩家行动，提供恢复草稿、重试和技术详情。
- 新增“用当前角色开启新剧情”、在新分支重生成或编辑行动后重试，以及“从这里继续”；保留原剧情作为候选。
- 改善开场导航与玩家称呼、当前角色名称、角色库封面和导入入口、页内换卡确认、玩家设定保存反馈与高级变量折叠；世界书支持搜索、启用状态筛选和复制到当前剧情再编辑，自动处理副本重名并同步编辑器。
- 时间线优先展示中文剧情摘要与可读时间，技术字段收进详情；新增剧情整理状态和来源回合跳转。
- 修复新版 DSH 中酒馆预设无法挂载的问题：persona 配置改用当前必填的 `prefix` 字段，避免挂载失败触发命令列表刷新循环。
- 适配 Harness 空白会话隐藏原生标签栏的场景：在既有 `conversation.input.dock` 开场区域提供“开场 / 世界书 / 用户设定/变量 / 脚本 / 提示词模板”切换入口，首条消息后回到对话旁的正式标签；不替换 Hero 或原生 Composer。
- 调整前端入口：Manager 保留角色库、角色卡和角色脚本（Scoped）；工作区与当前 Session 在同一层的 `conversation.view` 提供时间线、世界书、用户设定/变量、脚本（Global/Preset）和提示词模板，世界书视图同时提供共享资源编辑与当前 Session 引用选择。时间线继续使用 `sillytavern-events` 标签 ID，显示名改为“时间线”，保留原事件图与详情并可切换原事件编辑器。该变更只涉及前端入口，不新增 API-only 数据编辑器，工作区隔离、Session 数据 scope、处理与保存语义及角色卡默认世界书字段均不变；全局管理仍从 Session 角色菜单进入，已有服务需用户自行重启应用。
- 酒馆预设挂载 DSH 原生历史压缩、`/compact` 命令和工具结果裁剪，直接沿用宿主组件默认配置与 Host token meter；不在插件中复制原生压缩逻辑与阈值。
- 移除剩余的总 system 204,800 字符截断、历史消息 32,768 字符提取截断及模板/事件轮询历史的二次裁切；完整文本继续走 DSH 公共 LLM 请求链。模型上下文能力继续从 DSH 解析，SillyTavern 业务预算与尚未迁移的有状态 EJS 运行时额度保持独立。
- 移除提示词组装中的 32,768 字符段落上限及所有世界书位置共享的字符池，完整保留角色字段、开场白、模板输入、宏化关键词与脚本注入；修复长世界书已激活的 `@depth` 状态栏和选项被静默丢弃。世界书 token 预算及其他独立运行时上限保持不变。
- 无法解析模型上下文窗口时，世界书 token 回退预算由 2,048 提高到 250,000；可识别模型仍使用上下文比例、卡内 `token_budget` 与 Host cap 的最小值。
