...
- 每条时间线可独立放大、缩小、前后移动和回到总览，也可直接聚焦某个事件的时间。在亿年与毫秒并存的总览中，短于像素的区间保留可点击标记；标记的最小显示宽度不代表实际时长，局部放大后仍按原始数值的线性比例显示。未记录结束时间的图示终点固定为该轴全部标准化记录（含未分组记录）的最新已知坐标，不随视野改变；只有一个坐标时显示在右端。
